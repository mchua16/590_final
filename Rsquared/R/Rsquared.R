...
Rsquared <- function(x, y, xname, title) { #creates a function assigned as Rsquared
  ...
  num <- as.numeric(log10) #converts log10 values from dataset to numeric
  a <- ggplot() +
    ...
    ylab('log10(Absorbance)') + #changes y label to "log10(Absorbance)"
